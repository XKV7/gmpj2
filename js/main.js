...
function handleTitleClick() {
  ...
  } else if (hasSave && input.wasClicked(400 - btnW/2, startY + 70, btnW, btnH)) {
...
function handleStarterClick() {
  ...
  for (let i = 0; i < STARTERS.length; i++) {
    if (input.wasClicked(positions[i] - 100, 140, 200, 280)) {
...
function handleBattleClick() {
  ...
  switch (b.phase) {
    ...
    case 'MOVE_LEARN':      handleMoveLearnClick(b);     break;
    case 'PARTY_FULL_CATCH':handlePartyFullCatchClick(b); break;
...
function handleActionClick(b) {
  ...
  if (input.wasClicked(col2, row2, bw, bh)) {
    if (b.isWild) attemptFlee(b);
    else { b.result = 'lose'; b._endMsg = '배틀을 포기했다.'; b.phase = 'END'; }
  }
}
...
// ─── MOVE LEARN (기술 선택) ───
function handleMoveLearnClick(b) {
  const ml = b._moveLearning;
  if (!ml) { resumeAfterMoveLearn(b, null); return; }

  // Skip learning
  if (input.wasClicked(30, PANEL_Y + 222, 740, 38)) {
    resumeAfterMoveLearn(b, ml);
    return;
  }
  // Replace one of current 4 moves
  const bw = 720, bh = 50;
  ml.pocket.moves.forEach((m, i) => {
    const by = PANEL_Y + 42 + i * 56;
    if (input.wasClicked(40, by, bw, bh)) {
      ml.pocket.moves[i] = { ...MOVES_DB[ml.newMoveName], currentPp: MOVES_DB[ml.newMoveName].pp };
      resumeAfterMoveLearn(b, ml);
    }
  });
}

function resumeAfterMoveLearn(b, ml) {
  if (!ml || ml.remaining.length === 0) {
    b._moveLearning = null;
    if (b.result) b.phase = 'END';
    else b.phase = 'PLAYER_ACTION';
  } else {
    startMoveLearnChain(b, ml.remaining, ml.afterAll);
  }
}

function startMoveLearnChain(b, pendingList, afterAll) {
  if (pendingList.length === 0) {
    b._moveLearning = null;
    if (afterAll === 'END' || b.result) b.phase = 'END';
    else b.phase = 'PLAYER_ACTION';
    return;
  }
  ...
  b._moveLearning = {
    pocket: first.pocket,
    newMoveName: first.moveName,
    remaining: pendingList.slice(1),
    afterAll: afterAll || 'PLAYER_ACTION'
  };
}

// ─── PARTY FULL CATCH ───
function handlePartyFullCatchClick(b) {
  const caught = b._pendingCatch;
  if (!caught) { b.phase = 'END'; return; }

  // Release the caught pocket
  if (input.wasClicked(30, PANEL_Y + 220, 740, 38)) {
    b._pendingCatch = null;
    b.phase = 'END';
    return;
  }
  ...
  GS.player.party.forEach((p, i) => {
    ...
    const py = PANEL_Y + 48 + Math.floor(i / cols) * 72;
    if (input.wasClicked(px, py, 245, 62)) {
      if (GS.player.activePocketIdx === i) {
        const newActive = GS.player.party.findIndex((pp, ii) => ii !== i && pp.currentHp > 0);
        GS.player.activePocketIdx = newActive >= 0 ? newActive : 0;
        b.player = GS.player.party[GS.player.activePocketIdx];
      }
      caught.isWild = false;
      GS.player.party[i] = caught;
      b._pendingCatch = null;
      b.phase = 'END';
    }
  });
  ...
  if (input.wasClicked(250, PANEL_Y + 140, 300, 50)) {
    if (b.result === 'lose') {
      GS._gameOverParty = GS.player.party.map(p => ({
        ...
        level: p.level, maxHp: p.maxHp, status: p.status, types: p.species.types
      }));
...
// Perform one side's attack, collect messages, call afterFn
function doAttack(b, attacker, defender, move, afterFn) {
  const msgs = [];
  msgs.push(`${attacker.species.name}의 ${move.name}!`);

  const blocked = checkStatusBlock(attacker);
  if (blocked) { msgs.push(blocked); queueMessages(b, msgs, afterFn); return; }
  if (!checkAccuracy(move)) { msgs.push('기술이 빗나갔다!'); queueMessages(b, msgs, afterFn); return; }

  // Status-only moves (power === 0)
  if (move.category === 'status' || move.power === 0) {
    const fx = tryApplyEffect(move, defender);
    msgs.push(fx || '하지만 효과가 없었다...');
    queueMessages(b, msgs, afterFn);
    return;
  }
  ...
  msgs.push(`${defender.species.name}에게 ${damage}의 피해!`);

  const fx = tryApplyEffect(move, defender);
  if (fx) msgs.push(fx);
  if (defender.currentHp <= 0) msgs.push(`${defender.species.name}이(가) 쓰러졌다!`);
...
function executePlayerMove(b, move) {
  // PP consumed when committed
  if (!move.isStruggle) move.currentPp = Math.max(0, move.currentPp - 1);

  // Speed-based turn order; tie → 50/50
  const pSpd = b.player.stats.speed;
  const eSpd = b.enemy.stats.speed;
  const playerFirst = pSpd > eSpd ? true : eSpd > pSpd ? false : Math.random() < 0.5;

  if (playerFirst) {
    doAttack(b, b.player, b.enemy, move, () => {
      if (b.enemy.currentHp <= 0) { handleEnemyFaint(b); return; }
      const eMove = pickEnemyMove(b);
      doAttack(b, b.enemy, b.player, eMove, () => {
    ...
    const eMove = pickEnemyMove(b);
    doAttack(b, b.enemy, b.player, eMove, () => {
      if (b.player.currentHp <= 0) { handlePlayerFaint(b); return; }
      doAttack(b, b.player, b.enemy, move, () => {
        if (b.enemy.currentHp <= 0) handleEnemyFaint(b);
        else endOfTurn(b);
      });
    });
  }
...
function pickEnemyMove(b) {
  return aiChooseMove(b.enemy, b.player);
}

// Enemy attacks after a switch (always gets free attack)
function enemyAttackAfterSwitch(b) {
  const eMove = pickEnemyMove(b);
  doAttack(b, b.enemy, b.player, eMove, () => {
    if (b.player.currentHp <= 0) handlePlayerFaint(b);
    else endOfTurn(b);
...
function executeSwitch(b, newIdx) {
  ...
  queueMessages(b,
    [`${oldName}은(는) 돌아와!`, `${b.player.species.name}! 나가줘!`],
    () => enemyAttackAfterSwitch(b)
  );
}
...
function attemptFlee(b) {
  ...
  if (fleeSuccess(b.player, b.enemy)) {
    b.result = 'flee'; b._endMsg = '도망치는 데 성공했다!'; b.phase = 'END';
  } else {
    queueMessages(b, '도망치지 못했다!', () => {
      const eMove = pickEnemyMove(b);
      doAttack(b, b.enemy, b.player, eMove, () => {
        if (b.player.currentHp <= 0) handlePlayerFaint(b);
        else endOfTurn(b);
...
function attemptCatch(b, ballName) {
  ...
  if (Math.random() < rate) {
    ...
    if (GS.player.party.length < 6) {
      ...
      msgs.push('파티가 가득 찼다! 교체할 포켓을 선택하세요.');
      b._pendingCatch = b.enemy;
      queueMessages(b, msgs, () => { b.phase = 'PARTY_FULL_CATCH'; });
    }
  } else {
    msgs.push(`${b.enemy.species.name}이(가) 볼에서 빠져나왔다!`);
    queueMessages(b, msgs, () => {
      const eMove = pickEnemyMove(b);
      doAttack(b, b.enemy, b.player, eMove, () => {
        if (b.player.currentHp <= 0) handlePlayerFaint(b);
        else endOfTurn(b);
...
function endOfTurn(b) {
  ...
  if (b.player.currentHp <= 0) {
    if (msgs.length > 0) { queueMessages(b, msgs, () => handlePlayerFaint(b)); }
    else handlePlayerFaint(b);
    return;
  }
  if (b.enemy.currentHp <= 0) {
    if (msgs.length > 0) { queueMessages(b, msgs, () => handleEnemyFaint(b)); }
    else handleEnemyFaint(b);
    return;
  }
  if (msgs.length > 0) { queueMessages(b, msgs, () => { b.phase = 'PLAYER_ACTION'; }); }
  else b.phase = 'PLAYER_ACTION';
}
...
function handlePlayerFaint(b) {
  ...
  if (nextIdx === -1) {
    b.result = 'lose'; b._endMsg = '모든 포켓이 쓰러졌다!'; b.phase = 'END';
  } else {
...
function handleEnemyFaint(b) {
  ...
  for (const lu of levelUps) {
    ...
    for (const mn of lu.autoLearn) {
      msgs.push(`${b.player.species.name}은(는) ${MOVES_DB[mn].name}을(를) 배웠다!`);
    }
    for (const mn of lu.choiceMoves) {
      pendingMoveLearn.push({ pocket: b.player, moveName: mn });
    }
  }
  ...
  if (!b.isWild && b.trainer) {
    ...
    if (b.trainerPartyIdx < b.trainer.party.length) {
      ...
      msgs.push(`${b.trainer.name}이(가) 다음 포켓을 꺼냈다!`, `${b.enemy.species.name}! 나가줘!`);
      queueMessages(b, msgs, () => {
        if (pendingMoveLearn.length > 0) startMoveLearnChain(b, pendingMoveLearn, 'PLAYER_ACTION');
        else b.phase = 'PLAYER_ACTION';
      });
      return;
    } else {
  ...
  queueMessages(b, msgs, () => {
    if (pendingMoveLearn.length > 0) startMoveLearnChain(b, pendingMoveLearn, 'END');
    else b.phase = 'END';
  });
}
